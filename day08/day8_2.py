...
def add_antinodes_for_antennas(antenna1: tuple[int, int], antenna2: tuple[int, int]):
    ...
    while True:
        row1 = antenna1[0] - offset[0] * counter
        col1 = antenna1[1] - offset[1] * counter
        
        if 0 <= row1 < len(lines) and 0 <= col1 < len(lines[0]):
    ...
    while True:        
        row2 = antenna2[0] + offset[0] * counter
        col2 = antenna2[1] + offset[1] * counter
    
        if 0 <= row2 < len(lines) and 0 <= col2 < len(lines[0]):
